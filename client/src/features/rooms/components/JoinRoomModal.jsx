...
const animals = ['Fox', 'Otter', 'Panda', 'Lynx', 'Sparrow'];
  ...
  return (
    <Modal open={open} onClose={onCancel} title="Enter anonymous room">
      <div className="space-y-4">
        <div className="rounded-xl border border-border-default bg-brand-subtle p-4">
          <p className="text-[12px] font-medium uppercase tracking-[0.22em] text-text-secondary">Alias preview</p>
          <p className="mt-2 font-display text-[20px] font-medium leading-[1.35] text-text-primary">Guest {preview}</p>
        </div>
        {error || errorMessage ? (
